  ...
  return welcomeMessage.map((message, index) => {
    if (message.type === 'text') {
      return (
        <motion.div
          ...
          className={`absolute text-center container text-16 font-normal text-gray-600 ${currentIndex === index ? '' : 'pointer-events-none'
            }`}
        >
    ...
    if (message.type === 'mcq') {
      return (
        ...
        >
          {typeof message.data === 'object' && message.data.question && message.data.options &&(
            <div>
